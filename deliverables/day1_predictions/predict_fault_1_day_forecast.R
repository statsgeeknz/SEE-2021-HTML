...
#==================================================================================
#' Predictions for the 1 day forecasts
...
#' Args:
#' 
#' inputData - a dataset from the 1 day forecast webscraper [i.e. dataframe with 
...
predictionDay1 <- function(inputFile){
  ...
  modelObj <- readRDS("day1Model_120220.rds")
  inputDay <- 1
  inputData <- fromJSON(inputFile)
  inputData <- inputData[['1_day_forecast']]
  
  #' modify the data. Assume here that only have data for the day in question e.g.
  #' only day 1 forecast data corresponding to the day 1 model object
...
# some checks on data - roughly bounding these based on the modelling data
  ...
  if(any(is.na(inputData$wind_mean) | inputData$wind_mean < 5 | inputData$wind_mean > 49.25 )==TRUE) {
      print("One or more mean wind values are not supported (missing or more extreme than training data)")
      inputData$wind_mean <- sapply(inputData$wind_mean, rangeSnap, rangeToSnap = c(5, 49.25))
  }
  ...
  if(any(is.na(inputData$wind_gust_min) | inputData$wind_gust_min<5 | inputData$wind_gust_min>75 )==TRUE){
   ...
   if(any(is.na(inputData$wind_gust_max) | inputData$wind_gust_max<5 | inputData$wind_gust_max>100 )==TRUE){
     print("One or more min gust values are not supported (missing or more extreme than training data)")
     inputData$wind_gust_max <- sapply(inputData$wind_gust_max, rangeSnap, rangeToSnap = c(5, 100))
  }
  
  if(any(is.na(inputData$temp_max) | inputData$temp_max<(0) | inputData$temp_max>31 )==TRUE){
    print("One or more temperature max values are not supported (missing or more extreme than training data)")
    inputData$temp_max <- sapply(inputData$temp_max, rangeSnap, rangeToSnap = c(0, 31))
  }
  
  if(any(is.na(inputData$temp_min) | inputData$temp_min<(-10) | inputData$temp_min>17 )==TRUE){ 
    print("One or more temperature min values are not supported (missing or more extreme than training data)")
    inputData$temp_min <- sapply(inputData$temp_min, rangeSnap, rangeToSnap = c(-10, 17))
  }
  
  if(any(is.na(inputData$snow_depth) | inputData$snow_depth<0 | inputData$snow_depth>17.5 )==TRUE){
    print("One or more snow depth values are not supported (missing or more extreme than training data)")
    inputData$snow_depth <- sapply(inputData$snow_depth, rangeSnap, rangeToSnap = c(0, 17.5))
  }
  ...
  if(any(is.na(inputData$risk) | !(inputData$risk %in% c("Green", "Amber", "Red"))) ==TRUE){
