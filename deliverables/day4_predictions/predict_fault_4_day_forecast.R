...
#==================================================================================
#' Predictions for the 4 day forecasts
#' This function takes datasets from the webscraper (meteo-group forcast HTML) and
#' does a little formatting, before passing into model object fitted to the 4 day
#' forecast data. 
#' 
...
predictionDay4 <- function(inputFile){
...
# make sure the factors in the input data match possible levels from the modelling phase
  ...
  if(any(is.na(inputData$wind_gust) | inputData$wind_gust<15 | inputData$wind_gust>85 )==TRUE){
    print("One or more gust values are not supported (missing or more extreme than training data)")
    inputData$wind_gust <- sapply(inputData$wind_gust, rangeSnap, rangeToSnap = c(15, 85))
  }
  
  if(any(is.na(inputData$temp_max) | inputData$temp_max<(0) | inputData$temp_max>27 )==TRUE){
    print("One or more temperature max values are not supported (missing or more extreme than training data)")
    inputData$temp_max <- sapply(inputData$temp_max, rangeSnap, rangeToSnap = c(0, 27))
  }
