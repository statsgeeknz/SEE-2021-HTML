...
#==================================================================================
#' Predictions for the 5 day forecasts
#' This function takes datasets from the webscraper (meteo-group forcast HTML) and
#' does a little formatting, before passing into model object fitted to the 5 day
#' forecast data. 
#' 
...
predictionDay5 <- function(inputFile){
  ...
  modelObj <- readRDS("gamLSS_day5_130120.rds")
  inputDay <- 5
  
  #inputData <- read.csv(inputFile, header = T)
  inputData <- fromJSON(inputFile)
  inputData <- inputData[['5_day_forecast']]
...
# make sure the factors in the input data match possible levels from the modelling phase
  ...
  if(any(is.na(inputData$wind_mean) | inputData$wind_mean < 10 | inputData$wind_mean > 65 )==TRUE) {
    print("One or more mean wind values are not supported (missing or more extreme than training data)")
    inputData$wind_mean <- sapply(inputData$wind_mean, rangeSnap, rangeToSnap = c(10, 65))
  }
  

  
  if(any(is.na(inputData$wind_gust) | inputData$wind_gust<15 | inputData$wind_gust>80 )==TRUE){
    print("One or more gust values are not supported (missing or more extreme than training data)")
    inputData$wind_gust <- sapply(inputData$wind_gust, rangeSnap, rangeToSnap = c(15, 80))
  }
  
  if(any(is.na(inputData$temp_max) | inputData$temp_max<(0) | inputData$temp_max>26 )==TRUE){
    print("One or more temperature max values are not supported (missing or more extreme than training data)")
    inputData$temp_max <- sapply(inputData$temp_max, rangeSnap, rangeToSnap = c(0, 26))
  }
  
  if(any(is.na(inputData$temp_min) | inputData$temp_min<(0) | inputData$temp_min>14 )==TRUE){ 
    print("One or more temperature min values are not supported (missing or more extreme than training data)")
    inputData$temp_min <- sapply(inputData$temp_min, rangeSnap, rangeToSnap = c(0, 14))
  }
  
  if(any(is.na(inputData$snow_depth) | inputData$snow_depth<0 | inputData$snow_depth>8 )==TRUE){
    print("One or more snow depth values are not supported (missing or more extreme than training data)")
    inputData$snow_depth <- sapply(inputData$snow_depth, rangeSnap, rangeToSnap = c(0, 8))
  }
